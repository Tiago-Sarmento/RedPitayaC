	...
	while(1)
	{
		...
		for (trace_counts=0; trace_counts<50; trace_counts++)
		{
			...
			*     7 - External trigger 1*/
			...
			sprintf(payloadMain,"{\"channel\": \"vespa::failed_ind_Ext\", \"value\": %d, \"timestamp\": %f}",thisB,(unsigned)time(NULL)+ fracTime);
			mqtt_sendIndExt(payloadMain);
// 			mqtt_send(payloadMain); // can run at 50Hz without interrupting script
		...
		sprintf(payloadMain,"{\"channel\": \"vespa::failed_acc_Ext\", \"value\": %d, \"timestamp\": %u}",failedCountB,(unsigned)time(NULL));
		mqtt_sendAccExt(payloadMain); // can run at 50Hz without interrupting script	
	}
	// clean up
	fclose(fp);
	osc_fpga_exit();
